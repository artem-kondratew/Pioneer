...
import sys
import pickle
from rpi_camera import open_cap, cap_center
...
"""def find_center(marker) -> (int, int):
    print(marker)
    x = int(sum(marker[:, 0]) / len(marker))
    y = int(sum(marker[:, 1]) / len(marker))
    return x, y"""
...
def draw_marker_pose(img, marker_list):
    if not marker_list:
        return
    for marker in marker_list[0]:
        x, y = find_center(marker)
...
def img_proc(q: mp.Queue):
    ...
    while True:
        ...
        rvec, tvec = find_recs_tvecs(markerCorners, marker_size, mtx, dist)
        if rvec and tvec:
            cv.drawFrameAxes(image, mtx, dist, rvec[0], tvec[0], 0.01)
            cap_x, cap_y = float(tvec[0][0]).__round__(3), float(tvec[0][1]).__round__(3)
            cap_x = np.array([[cap_x], [cap_y]])
            print('COORDINATES:\n', cap_x)
