...
if __name__ == "__main__":
    ...
    cap_pipeline = (f'udpsrc caps="application/x-rtp,media=(string)video,encoding-name=(string)JPEG" port=4000 !'
                    f'rtpjpegdepay ! jpegdec ! queue ! videoconvert ! video/x-raw,width={width},height={height},'
                    f'format=BGR ! appsink')

    cap = cv.VideoCapture(cap_pipeline, cv.CAP_GSTREAMER)

    # Load image
    image = cv.imread('rotors.jpg')
